...
/** Generous — a genuine visitor submits once, twice if they made a mistake. */
...
export async function POST(req: Request) {
  try {
    ...
    const limit = await rateLimitShared(`lead:ip:${ip}`, PER_IP.limit, PER_IP.windowMs);
    if (!limit.allowed) {
      return NextResponse.json(
    ...
    const { id } = await createLead(parsed.data, "web", {
      ip,
    ...
    // A lead that cannot be stored must not look stored. The form keeps the
    // visitor's answers and offers WhatsApp instead.
    console.error("[leads] could not store enquiry", err);
    return NextResponse.json(
